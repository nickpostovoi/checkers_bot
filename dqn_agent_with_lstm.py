...
class DQN_agent:
    def __init__(self, state_size, action_size, num_past_states=6, initial_epsilon=1.0):
    ...
    def _build_model(self):
        ...
        model = Sequential()
        model.add(LSTM(256, input_shape=(self.num_past_states + 1, self.state_size), return_sequences=True))
        model.add(LSTM(128, return_sequences=False))
        model.add(Dense(512, input_dim=self.state_size, activation='relu', kernel_regularizer=l2(0.01), kernel_initializer=init))
        model.add(Dense(1024, activation='relu', kernel_regularizer=l2(0.01), kernel_initializer=init))
        model.add(Dense(2048, activation='relu', kernel_regularizer=l2(0.01), kernel_initializer=init))
        model.add(Dense(1024, activation='relu', kernel_regularizer=l2(0.01), kernel_initializer=init))
    ...
    def act(self, state, legal_moves):
        ...
        if np.random.rand() <= self.epsilon:
            ...
            all_possible_actions = set(range(self.action_size))  
            illegal_moves = all_possible_actions - set(legal_moves)
    ...
    def replay(self, batch_size):
        ...
        states = np.reshape(states, [len(minibatch), self.num_past_states + 1, self.state_size])
        next_states = np.reshape(next_states, [len(minibatch), self.num_past_states + 1, self.state_size])
